...
import { Container, Header, Left, Body, Right, Title, Spinner, ActionSheet, Root, Fab } from 'native-base';
...
export default class ListOfQuiz extends React.Component {
  ...
  alert_delete(index, name) {
    Alert.alert(
      ...
      [
        ...
        {
          text: "مسح", onPress: () => this.delete_quiz(index)
        }
      ],
      { cancelable: false }
    );
  ...
  delete_quiz(index) {
    ...
    axios.post(basic.url + `admin/delete_quiz.php`,
      data_to_send
    ).then((res) => {
  ...
  get_quiz = (exam_quiz_index) => {
    ...
    axios
      .post(
        basic.url + `admin/select_total_quiz.php`,
        data_to_send
      )
  ...
  update_show_to_public(pu_exam_id) {
    // this.setState({disabled: true});
    ...
    axios
      .post(
        basic.url + `admin/update_quiz_show_to_public.php`,
        data_to_send
      )
      .then((res) => {
        if (res.status == 200) {
  ...
  update_show_to_answer(pu_exam_id) {
    // this.setState({disabled: true});
    let generation_id = this.props.navigation.getParam('generation_id');
    let collection_id = this.props.navigation.getParam('collection_id');
    ...
    axios
      .post(
        basic.url + `admin/update_exam_show_to_answer.php`,
        data_to_send
      )
      .then((res) => {
        if (res.status == 200) {
                                ...
                                ActionSheet.show(
                                  ...
                                  buttonIndex => {
                                    if (buttonIndex == 0) {
                                      ...
                                      this.props.navigation.navigate('SolvedStudents', {
                                        exam_id: item.quiz_id,
                                        exam_name: item.quiz_name,
                                        test_id: 2,
                                        collection_id: this.props.navigation.getParam('collection_id'),
                                      ...
                                      this.props.navigation.navigate('NotSolvedStudents', {
                                        exam_id: item.quiz_id,
                                        generation_id: this.state.generation_id,
                                        test_id: 2,
                                        collection_id: this.props.navigation.getParam('collection_id'),
                                      ...
                                      this.props.navigation.navigate('ExamQuestions', {
